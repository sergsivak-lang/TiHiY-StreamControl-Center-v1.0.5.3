...
public sealed class AimpOverlayServer : IAsyncDisposable
{
    ...
    public Task StartAsync(int preferredPort)
    {
        ...
        for (var offset = 0; offset < 10; offset++)
        {
            try
            {
                var listener = new TcpListener(IPAddress.Loopback, firstPort + offset);
                listener.Start();
    ...
    private async Task HandleClientAsync(TcpClient client, CancellationToken token)
    {
        ...
        using (var reader = new StreamReader(stream, Encoding.UTF8, false, 4096, true))
        {
            ...
            if (path == "/api/cover")
            {
                ...
                var bytes = cover.Data.Length == 0 ? Encoding.UTF8.GetBytes(PlaceholderSvg) : cover.Data;
                var type = cover.Data.Length == 0 ? "image/svg+xml; charset=utf-8" : cover.ContentType;
                await RespondBytesAsync(stream, type, bytes, token, cover.Data.Length == 0 ? 0 : 86400).ConfigureAwait(false);
                return;
            }
            if (path == "/api/control")
            {
    ...
    {
        var width = ClampInt(uri, "width", 620, 380, 1400);
        var height = ClampInt(uri, "height", 112, 82, 260);
        var cover = ClampInt(uri, "cover", 88, 54, 180);
        var bg = ClampDouble(uri, "bg", 0.82, 0, 1);
        var opacity = ClampDouble(uri, "opacity", 1, 0.15, 1);
        var radius = ClampInt(uri, "radius", 11, 0, 40);
        ...
        return WidgetTemplate
            ...
            .Replace("__BG2__", Math.Max(0, bg - 0.04).ToString("0.00", CultureInfo.InvariantCulture))
            .Replace("__OPACITY__", opacity.ToString("0.00", CultureInfo.InvariantCulture))
            .Replace("__RADIUS__", radius.ToString(CultureInfo.InvariantCulture));
    }
    ...
    private static int ClampInt(Uri uri, string name, int fallback, int min, int max) =>
        int.TryParse(GetQuery(uri, name), out var value) ? Math.Clamp(value, min, max) : fallback;

    private static double ClampDouble(Uri uri, string name, double fallback, double min, double max) =>
        double.TryParse(GetQuery(uri, name), NumberStyles.Float, CultureInfo.InvariantCulture, out var value) ? Math.Clamp(value, min, max) : fallback;
...
<div class="preview"><iframe id="frame"></iframe></div><input id="url" class="url" readonly><button id="copy">СКОПІЮВАТИ URL ДЛЯ OBS</button><div class="note">Рекомендована висота Browser Source в OBS: 140–180 px. Фон сторінки прозорий.</div></div><script>
const bg=document.getElementById('bg'),op=document.getElementById('op'),h=document.getElementById('h'),w=document.getElementById('w'),c=document.getElementById('c'),frame=document.getElementById('frame'),url=document.getElementById('url');function update(){document.getElementById('bgv').textContent=bg.value+'%';document.getElementById('opv').textContent=op.value+'%';document.getElementById('hv').textContent=h.value+' px';document.getElementById('wv').textContent=w.value+' px';document.getElementById('cv').textContent=c.value+' px';const u='http://127.0.0.1:__PORT__/overlay/now-playing?bg='+(+bg.value/100).toFixed(2)+'&opacity='+(+op.value/100).toFixed(2)+'&height='+h.value+'&width='+w.value+'&cover='+c.value;url.value=u;frame.src=u+'&t='+Date.now()}[bg,op,h,w,c].forEach(x=>x.addEventListener('input',update));document.getElementById('copy').addEventListener('click',async()=>{await navigator.clipboard.writeText(url.value)});update();
</script></body></html>
""";
