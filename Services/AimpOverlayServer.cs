...
/// <summary>
/// Tiny dedicated server for the AIMP Now Playing widget. It remains idle when OBS
/// is not requesting it and uses cached metadata plus the exact AIMP Remote timeline.
/// </summary>
public sealed class AimpOverlayServer : IAsyncDisposable
{
    ...
    private async Task HandleClientAsync(TcpClient client, CancellationToken token)
    {
        ...
        using (var reader = new StreamReader(stream, Encoding.UTF8, false, 4096, true))
        {
            ...
            // Local-only compatibility endpoint. It is not exposed anywhere in the stream widget.
            if (path == "/api/control")
            {
...
html,body{margin:0;background:transparent;overflow:hidden;font-family:"Segoe UI",sans-serif;color:#f4f8ff}
#card{position:absolute;left:18px;bottom:18px;width:min(720px,calc(100vw - 36px));min-height:152px;box-sizing:border-box;padding:14px 16px;display:grid;grid-template-columns:128px minmax(0,1fr);gap:16px;border-radius:12px;background:linear-gradient(135deg,rgba(3,18,34,.94),rgba(5,38,64,.90));border:1px solid rgba(55,169,255,.58);border-left:5px solid #FFD329;box-shadow:0 12px 35px rgba(0,0,0,.35);opacity:0;transform:translateY(14px);transition:.25s}
#card.on{opacity:1;transform:none}.cover{width:128px;height:128px;border-radius:9px;object-fit:cover;background:#051221;border:1px solid rgba(255,211,41,.6)}.content{min-width:0;display:flex;flex-direction:column;justify-content:center}.label{font-size:11px;font-weight:900;letter-spacing:1.8px;color:#FFD329}.title{font-size:25px;font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:5px}.artist{font-size:17px;font-weight:700;color:#bfe9ff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.album{font-size:12px;color:#8eaabb;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px}.timeline{display:grid;grid-template-columns:auto 1fr auto;gap:8px;align-items:center;margin-top:14px}.time{font:11px Consolas,monospace;color:#a9c2d2}.bar{height:6px;border-radius:999px;background:rgba(255,255,255,.14);overflow:hidden}.fill{height:100%;width:0;background:linear-gradient(90deg,#24a7ff,#FFD329);transition:width .2s linear}
</style></head><body><div id="card"><img id="cover" class="cover" alt="cover"><div class="content"><div class="label">AIMP • ЗАРАЗ ГРАЄ</div><div id="title" class="title"></div><div id="artist" class="artist"></div><div id="album" class="album"></div><div class="timeline"><span id="pos" class="time">0:00</span><div class="bar"><div id="fill" class="fill"></div></div><span id="dur" class="time">0:00</span></div></div></div><script>
let model=null,lastFetch=0,lastCover=-1;const $=id=>document.getElementById(id);function fmt(v){v=Math.max(0,Number(v)||0);const h=Math.floor(v/3600),m=Math.floor((v%3600)/60),s=Math.floor(v%60);return h>0?h+':'+String(m).padStart(2,'0')+':'+String(s).padStart(2,'0'):m+':'+String(s).padStart(2,'0')}function renderProgress(){if(!model){requestAnimationFrame(renderProgress);return}let p=Number(model.positionSeconds)||0;if(model.isPlaying)p+=(performance.now()-lastFetch)/1000;const d=Number(model.durationSeconds)||0;if(d>0)p=Math.min(p,d);$('pos').textContent=fmt(p);$('dur').textContent=fmt(d);$('fill').style.width=(d>0?Math.max(0,Math.min(100,p/d*100)):0)+'%';requestAnimationFrame(renderProgress)}async function update(){try{const m=await(await fetch('/api/now-playing',{cache:'no-store'})).json();model=m;lastFetch=performance.now();$('card').classList.toggle('on',!!m.active);$('title').textContent=m.title||'';$('artist').textContent=m.artist||'';$('album').textContent=m.album||'';if(Number(m.coverVersion)!==lastCover){lastCover=Number(m.coverVersion)||0;$('cover').src='/api/cover?v='+lastCover}}catch(e){}}setInterval(update,1000);update();requestAnimationFrame(renderProgress);
</script></body></html>
""";
