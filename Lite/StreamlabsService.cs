...
public sealed class StreamlabsService : IAsyncDisposable
{
    ...
    public async Task AuthorizeAsync(string clientId, string clientSecret, CancellationToken token = default)
    {
        ...
        if (!result.TryGetValue("state", out var returnedState) || !string.Equals(returnedState, state, StringComparison.Ordinal))
            throw new InvalidOperationException("Streamlabs OAuth: невірний state.");
        if (!result.TryGetValue("code", out var code) || string.IsNullOrWhiteSpace(code))
            throw new InvalidOperationException("Streamlabs не повернув код авторизації.");
    ...
    public async Task<IReadOnlyList<JsonObject>> GetRecentDonationsAsync(int limit = 20, CancellationToken token = default)
    {
        ...
        return array?.OfType<JsonObject>().ToList() ?? Array.Empty<JsonObject>();
    }

    public Task SendTestAlertAsync(string type = "follow", CancellationToken token = default) =>
        PostAlertCommandAsync("alerts/send_test_alert", new Dictionary<string,string> { ["type"] = type }, token);
    ...
    public async Task DisconnectAsync()
    {
        ...
        try { if (_socket.Connected) await _socket.DisconnectAsync().ConfigureAwait(false); } catch { }
        _socket.Dispose();
        _socket = null;
