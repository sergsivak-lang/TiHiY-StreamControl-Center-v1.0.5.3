...
public sealed class StreamlabsService : IAsyncDisposable
{
    ...
    private volatile bool _socketIoReady;
    ...
    public bool IsConnected => _socket?.State == WebSocketState.Open && _socketIoReady;
        ...
        while (!token.IsCancellationRequested)
        {
            try
            {
                if (_socket is null || _socket.State != WebSocketState.Open)
                    await ConnectSocketOnceAsync(token).ConfigureAwait(false);

                await ReceiveLoopAsync(token).ConfigureAwait(false);
                if (_socketIoReady) retryMs = 1000;
            }
            catch (OperationCanceledException) { break; }
            catch (Exception ex)
            {
                _logger.Error("Streamlabs socket", ex);
            }
            ...
            try { await Task.Delay(retryMs, token).ConfigureAwait(false); }
            catch { break; }
            retryMs = Math.Min(5000, retryMs + 750);
        }
    }
    ...
    private async Task ReceiveLoopAsync(CancellationToken token)
    {
        ...
        try
        {
            while (!token.IsCancellationRequested && _socket?.State == WebSocketState.Open)
            {
                ...
                do
                {
                    ...
                    {
                        _logger.Info($"Streamlabs socket closed by server: {_socket.CloseStatus} {_socket.CloseStatusDescription}");
                        return;
                    }
                ...
                // EIO3 pong response to our client ping.
                if (frame.StartsWith("3", StringComparison.Ordinal))
                {
                ...
                {
                    _logger.Info("Streamlabs Socket.IO namespace disconnected by server.");
                    return;
                }

                if (frame.StartsWith("42", StringComparison.Ordinal) && frame.Length > 2)
                {
                    if (!_socketIoReady)
                    {
                        _socketIoReady = true;
        ...
        finally
        {
            ...
            if (heartbeatTask is not null)
            {
                ...
                catch (Exception ex) { _logger.Error("Streamlabs heartbeat", ex); }
            }
        }
    }

    private async Task HeartbeatLoopAsync(TimeSpan pingInterval, TimeSpan pingTimeout, CancellationToken token)
    {
        // Engine.IO v3 heartbeat direction: client -> ping, server -> pong.
        while (!token.IsCancellationRequested && _socket?.State == WebSocketState.Open)
        {
            await Task.Delay(pingInterval, token).ConfigureAwait(false);
            if (token.IsCancellationRequested || _socket?.State != WebSocketState.Open) break;

            var sentAt = DateTime.UtcNow;
            _lastPongUtc = DateTime.MinValue;
            await SendSocketTextAsync("2", token).ConfigureAwait(false);

            var deadline = sentAt + pingTimeout;
            while (!token.IsCancellationRequested && DateTime.UtcNow < deadline)
            {
                if (_lastPongUtc >= sentAt) break;
                await Task.Delay(100, token).ConfigureAwait(false);
            }

            if (_lastPongUtc < sentAt)
            {
                _logger.Info($"Streamlabs heartbeat timeout: pong not received within {pingTimeout.TotalMilliseconds:0}ms.");
    ...
    private static (TimeSpan PingInterval, TimeSpan PingTimeout) ParseEngineIoHandshake(string frame)
    {
        const int defaultIntervalMs = 25_000;
        const int defaultTimeoutMs = 5_000;
        try
        {
            if (frame.Length <= 1) return (TimeSpan.FromMilliseconds(defaultIntervalMs), TimeSpan.FromMilliseconds(defaultTimeoutMs));
            using var doc = JsonDocument.Parse(frame[1..]);
            var root = doc.RootElement;
            var interval = root.TryGetProperty("pingInterval", out var pi) && pi.TryGetInt32(out var i) ? i : defaultIntervalMs;
            var timeout = root.TryGetProperty("pingTimeout", out var pt) && pt.TryGetInt32(out var t) ? t : defaultTimeoutMs;
            interval = Math.Clamp(interval, 5_000, 120_000);
            timeout = Math.Clamp(timeout, 2_000, 120_000);
            return (TimeSpan.FromMilliseconds(interval), TimeSpan.FromMilliseconds(timeout));
        }
        catch
        {
            return (TimeSpan.FromMilliseconds(defaultIntervalMs), TimeSpan.FromMilliseconds(defaultTimeoutMs));
        }
    }
    ...
    private async Task SendSocketTextAsync(string text, CancellationToken token)
    {
        ...
        await _socketSendLock.WaitAsync(token).ConfigureAwait(false);
        try
        {
        ...
        try
        {
            ...
            if (messages.ValueKind == JsonValueKind.Array)
            {
                foreach (var item in messages.EnumerateArray())
                    EventReceived?.Invoke(this, new StreamlabsEvent { For = platform, Type = type, EventId = eventId, Payload = item.Clone() });
            }
            else
            {
                EventReceived?.Invoke(this, new StreamlabsEvent { For = platform, Type = type, EventId = eventId, Payload = messages.Clone() });
            }
        }
        catch (Exception ex)
        {
            _logger.Error("Streamlabs event parse", ex);
        }
    }
    ...
    public Task SendTestAlertAsync(string type = "follow", CancellationToken token = default) =>
        PostAlertCommandAsync("alerts/send_test_alert", new Dictionary<string, string> { { "type", type }, { "platform", "twitch" } }, token);
    ...
    private async Task PostAlertCommandAsync(string endpoint, Dictionary<string, string>? parameters, CancellationToken token)
    {
        ...
        using var req = new HttpRequestMessage(HttpMethod.Post, "https://streamlabs.com/api/v2.0/" + endpoint);
        req.Headers.Authorization = new AuthenticationHeaderValue("Bearer", access);
    ...
    public static void OpenDeveloperPage() =>
        Process.Start(new ProcessStartInfo("https://streamlabs.com/dashboard#/settings/api-settings") { UseShellExecute = true });
    ...
    public async Task DisconnectAsync()
    {
        ...
        if (_socketLoop is not null)
        {
            try { await _socketLoop.ConfigureAwait(false); } catch { }
        }
        _socketLoop = null;
        _socketCts?.Dispose();
        _socketCts = null;
